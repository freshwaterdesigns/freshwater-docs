...
- **Video block — Link URL (follow-up):** Avoid putting `video.alt` inside the stretch link (can render as media markup and escape `.visually-hidden`, showing a broken image). Use `aria-label` with `sections.slideshow.slide` only. When **Autoplay** is on, stack the link above the video with `pointer-events: none` on the video so the URL is clickable; when autoplay is off, keep the video above the link so native controls work (clicking the video surface cannot also follow the URL).
