...
- **Dawn inherited foreground opacity:** `body` and every color scheme root (`.color-*` from theme settings) now use `color: rgba(var(--color-foreground), 1)` under `body.fresh`, matching the existing per-component overrides so general text is not stuck at Dawn’s 0.75 inherited opacity. `assets/0-freshwater.css.liquid`.
