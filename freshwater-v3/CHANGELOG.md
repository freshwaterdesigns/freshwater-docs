...
### Changed
- **Cart — free shipping bar:** New theme setting **Show free shipping bar** (`fresh_free_shipping_bar_enabled`, default on). When off, the drawer free shipping block is hidden. When on, **Free Shipping Threshold** can be `0` to show full progress and the unlocked message for any non-empty cart (no longer “0 hides the bar”). `config/settings_schema.json`, `snippets/0-cart-drawer.liquid`.
