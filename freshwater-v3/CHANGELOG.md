...
### Fixed
- **Multi Column (FW) — block margin bottom ignored:** `--fresh-block-margin-bottom` was only set globally to `20px` in `0-freshwater.css.liquid` and never overridden by the section setting (`fresh_graphic_margin_bottom--md/sm`, default 0px). Added the CSS custom property assignment to the section-level `#freshSection--` rule in both desktop and mobile media queries in `sections/0-multi-column-1.liquid`.
