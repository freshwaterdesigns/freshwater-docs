...
### Added
- **Video blocks — play button overlay:** New per-breakpoint schema toggles (`Desktop: Show Play Overlay` / `Mobile: Show Play Overlay`) on all video blocks. When enabled, a circular SVG play icon (YouTube-style) overlays the video and hides once the video begins playing. Color pickers (`Play Button Background`, `Play Button Arrow`) are shared across breakpoints. Available in: hero, hero-2, multi-column, two-column, one-column, main-product sections.

### Fixed
- **Video blocks — mute button visibility control:** Added per-breakpoint `Desktop: Show Mute Button` / `Mobile: Show Mute Button` toggles (default on) to all video block schemas. The JS now checks `data-mute-button-md/sm` on the container before injecting the mute button, allowing it to be suppressed per breakpoint without removing the native-controls check.
